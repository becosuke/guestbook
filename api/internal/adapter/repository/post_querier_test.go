...
func TestGetPost(t *testing.T) {
	...
	t.Run("existing post", func(t *testing.T) {
		id := newUUID()
		body := "test body"
		post := domain.NewPost(domain.NewPostID(id), domain.NewPostBody(body), time.Time{}, time.Time{}, nil)
	...
	t.Run("deleted post is still returned by GetPost", func(t *testing.T) {
		id := newUUID()
		post := domain.NewPost(domain.NewPostID(id), domain.NewPostBody("to be deleted"), time.Time{}, time.Time{}, nil)
		err := testCommander.CreatePost(ctx, post)
		require.NoError(t, err)
...
func TestRangePosts(t *testing.T) {
	...
	t.Run("without cursor", func(t *testing.T) {
		...
		for i := range ids {
			ids[i] = newUUID()
			post := domain.NewPost(domain.NewPostID(ids[i]), domain.NewPostBody("body "+ids[i][:8]), time.Time{}, time.Time{}, nil)
			err := testCommander.CreatePost(ctx, post)
			require.NoError(t, err)
